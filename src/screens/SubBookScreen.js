...
import { Alarm, Task } from '../models/types';
import Tarjeta from '../components/Tarjeta';
import Modal from '../components/Modal';
import { SOUNDS } from '../constants/sounds';

function SubBookScreen() {
  const { id } = useParams();
  ...
  const handleClickChild = (child) => {
    if (child.type === 'alarm') navigate(`/alarm/${child.id}`);
  ...
  if (!subbook) return <div className="container">Cargando...</div>;

  return (
    <div className="container">
      <button className="secundario" onClick={() => navigate(-1)}>← Volver</button>
      <h1>{subbook.name}</h1>
      {subbook.image && <img src={subbook.image} alt="Portada" style={{ width: '100%', maxHeight: '200px', objectFit: 'cover', borderRadius: '8px', marginBottom: '16px' }} />}
      <p style={{ color: '#E0E0E0', marginBottom: '20px' }}>{subbook.description || 'Sin descripción'}</p>
      ...
      {children.length === 0 && <p style={{ color: '#9E9E9E' }}>No hay alarmas ni tareas.</p>}
      ...
      {showModal && (
        <Modal onClose={() => setShowModal(false)}>
          <h3>{modalType === 'alarm' ? 'Nueva Alarma' : 'Nueva Tarea'}</h3>
